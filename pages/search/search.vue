<template>
	<view class="content">
		<view class="search-box">
			<mSearch class="mSearch-input-box" :mode="2" 
			button="inside" :placeholder="defaultKeyword"
			@search="doSearch(false)" @input="inputChange" 
			@confirm="doSearch(false)" v-model="keyword"/>
		</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			//加载默认搜索关键字
			loadDefaultKeyword() {
				//定义默认搜索关键字，可以自己实现ajax请求数据再赋值,用户未输入时，
				//以水印方式显示在输入框，直接不输入内容搜索会搜索默认关键字
				this.defaultKeyword = "输入基金经理";
			},
			...
			loadHotKeyword() {
				...
				this.hotKeywordList = ['键盘', 
				'鼠标1212121212121212121212', 
				'显示21212121212121器',
				'电脑21212121212121主机',
				'蓝牙音箱', 
				 '笔记21212121212121本电脑', 
			...
			inputChange(event) {
				...
				uni.request({
					url: "http://127.0.0.1/v1/api/search/tips?key=" + keyword, //仅为示例
					success: (res) => {
						this.keywordList = [];
			...
			drawCorrelativeKeyword(keywords, keyword) {
				...
				for (var i = 0; i < len; i++) {
					var row = keywords[i];
					//定义高亮#9f9f9f
					var html = row.replace(keyword, "<span style='color: #ff0000;'>" + keyword + "</span>");
					html = '<div>' + html + '</div>';
					var tmpObj = {
						keyword: row,
						htmlStr: html
			...
			doSearch(keyword) {
				...
				uni.navigateTo({
					url: '/pages/FundDetails/FundDetails'+"?index=" + keyword
				});
				uni.showToast({
					title: keyword,
					icon: 'none',
